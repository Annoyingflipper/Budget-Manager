...
Append to `src/components/LineItemRow.test.tsx`, matching the render helper already used in that file:

```tsx
describe('due date control', () => {
  it('offers to set a due date when none is set', () => {
    renderRow({ dueOn: null });
    expect(screen.getByRole('button', { name: 'Set due date' })).toBeInTheDocument();
  });

  it('shows the due date when one is set', () => {
    renderRow({ dueOn: '2026-09-01' });
    expect(screen.getByLabelText('Due date')).toHaveValue('2026-09-01');
  });

  it('saves a due date through updateLineItem', async () => {
    renderRow({ dueOn: '2026-09-01' });
    const input = screen.getByLabelText('Due date');
    await userEvent.clear(input);
    ...
    expect(updateLineItem).toHaveBeenCalledWith(
      expect.any(Number),
      expect.objectContaining({ dueOn: '2026-09-05' }),
    );
  });
  ...
  it('clears the due date', async () => {
    ...
    expect(updateLineItem).toHaveBeenCalledWith(
      expect.any(Number),
      expect.objectContaining({ dueOn: null }),
    );
  });

  it('tones an unpaid past-due row as overdue', () => {
    vi.setSystemTime(new Date(2026, 7, 24)); // 24 Aug 2026, local
    renderRow({ dueOn: '2026-08-01', paidOn: null });
    expect(screen.getByLabelText('Due date').className).toContain('border-negative');
  });

  it('does not tone a paid row as overdue', () => {
    vi.setSystemTime(new Date(2026, 7, 24));
    renderRow({ dueOn: '2026-08-01', paidOn: '2026-08-02' });
    expect(screen.getByLabelText('Due date').className).not.toContain('border-negative');
  });
});
```

Note `vi.setSystemTime` uses the local-time component constructor, not a date string, for the same UTC reason as everywhere else. Add `vi.useFakeTimers()` / `vi.useRealTimers()` around those two tests if the file does not already manage timers.
