...
- **v1.9+** — Paid-dates follow-ups: per-item due day (to power overdue highlighting) or a `paid_on` column in CSV export. Alternatively: patterns over time — spending-breakdown donut, trend-over-time chart, or pick-any-two-month comparison (deferred from v1.6). Or budget sharing with another user (read-only or co-edit). Pick one per cycle.
